...
class WordServer():
	...
	'''This function is the constructor for the WordServer class:
	...
	WordServer.min_frequence = 0.5

	# We initialize the parameters
	self.parent = parent
	self.word = word
	# If the frequence is correct, we keep it
	if frequence >= WordServer.min_frequence:
	    self.frequence = frequence
	# If it's negative, we take a random smaller frequence
	elif frequence <= -1.0 * WordServer.min_frequence:
	    self.frequence = -1.0*WordServer.min_frequence*random.random()
	# FIXME we have to handle the case where the frequence is not correct

	print '[PID: %5i] New word server: (%s, %f)' % (os.getpid(), self.word, self.frequence)
